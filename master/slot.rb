module Slot
  def add_slot
    ...
    @bot.api.send_message(chat_id: CONFIG['master_id'], text: "Слот на #{@text} создан.")
    show_options
  end
  ...
  def remove_slot
    ...
    @bot.api.send_message(chat_id: CONFIG['master_id'], text: 'Какой слот хотите удалить?', reply_markup: markup)
  end

  def add_apointment
    kb = MongoClient.active_slots.map { |s| [Telegram::Bot::Types::KeyboardButton.new(text: "Записать на #{s['date']} #{s['time']}")] }
    markup = Telegram::Bot::Types::ReplyKeyboardMarkup.new(keyboard: kb, one_time_keyboard: true)
    @bot.api.send_message(chat_id: CONFIG['master_id'], text: 'Выберите слот для записи?', reply_markup: markup)
  end
  ...
  def ask_for_new_time(slot)
    ...
    @bot.api.send_message(chat_id: CONFIG['master_id'], text: 'Выберите новое время', reply_markup: markup)
  end

  def ask_for_change
    kb = MongoClient.active_slots.map { |s| [Telegram::Bot::Types::KeyboardButton.new(text: "Изменить слот #{s['date']} #{s['time']}")] }
    markup = Telegram::Bot::Types::ReplyKeyboardMarkup.new(keyboard: kb, one_time_keyboard: true)
    @bot.api.send_message(chat_id: CONFIG['master_id'], text: 'Какой слот хотите изменить?', reply_markup: markup)
  end
  ...
  def push_schedule
    ...
    MongoClient.show_users.each do |user|
      next if user['id'] == CONFIG['master_id']
  ...
  def choose_month
    ...
    markup = Telegram::Bot::Types::ReplyKeyboardMarkup.new(keyboard: kb, one_time_keyboard: true)
    @bot.api.send_message(chat_id: CONFIG['master_id'], text: 'Выберите месяц', reply_markup: markup)
  end
  ...
  def choose_date
    ...
    @bot.api.send_message(chat_id: CONFIG['master_id'], text: 'Выберите дату', reply_markup: markup)
  end

  def choose_hour
    kb = (8..21).map { |h| Telegram::Bot::Types::KeyboardButton.new(text: "#{@text} #{h}") }
    markup = Telegram::Bot::Types::ReplyKeyboardMarkup.new(keyboard: kb.each_slice(3).to_a, one_time_keyboard: true)
    @bot.api.send_message(chat_id: CONFIG['master_id'], text: 'Выберите час', reply_markup: markup)
  end

  def choose_minute
    kb = %w[00 30].map { |m| [Telegram::Bot::Types::KeyboardButton.new(text: "#{@text}:#{m}")] }
    markup = Telegram::Bot::Types::ReplyKeyboardMarkup.new(keyboard: kb, one_time_keyboard: true)
    @bot.api.send_message(chat_id: CONFIG['master_id'], text: 'Выберите минуты', reply_markup: markup)
  end
end